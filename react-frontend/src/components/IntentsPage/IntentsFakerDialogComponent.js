...
import intentsFakerFactory from "./intentsFakerFactory";
...
const IntentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = intentsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("intents").create(elem));
...
export default connect(null, mapDispatch)(IntentsFakerDialogComponent);
